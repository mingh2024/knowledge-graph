...
function makeLabel(name){
  var c=document.createElement("canvas");c.width=420;c.height=80;
  var ctx=c.getContext("2d");ctx.clearRect(0,0,420,80);
  ...
  if(name.length<=5){ctx.font='bold 28px "Microsoft YaHei","PingFang SC",sans-serif';ctx.fillText(name,210,40);}
  else if(name.length<=9){var mi=Math.ceil(name.length/2);ctx.font='bold 24px "Microsoft YaHei","PingFang SC",sans-serif';ctx.fillText(name.slice(0,mi),210,28);ctx.fillText(name.slice(mi),210,52);}
  else{ctx.font='bold 20px "Microsoft YaHei","PingFang SC",sans-serif';for(var i=0;i<Math.ceil(name.length/5)&&i<3;i++)ctx.fillText(name.slice(i*5,(i+1)*5),210,18+i*24);}
  return c;
}
function makeRelIcon(type,color){
  var c=document.createElement("canvas");c.width=28;c.height=28;
  var ctx=c.getContext("2d");var cx=14,cy=14,r=11;
  var g3d=color||d3D();var grad=ctx.createRadialGradient(cx-3,cy-3,1,cx,cy,r);
  grad.addColorStop(0,g3d[0]);grad.addColorStop(0.6,g3d[1]);grad.addColorStop(1,g3d[2]);
  ctx.beginPath();ctx.arc(cx,cy,r,0,Math.PI*2);ctx.fillStyle=grad;ctx.fill();
  ctx.strokeStyle="rgba(255,255,255,0.9)";ctx.fillStyle="rgba(255,255,255,0.9)";ctx.lineWidth=1.5;ctx.beginPath();
  switch(type){
    case"\u524d\u7f6e\u4e8e":ctx.moveTo(cx+7,cy);ctx.lineTo(cx-5,cy-7);ctx.lineTo(cx-5,cy+7);ctx.closePath();ctx.fill();break;
    case"\u4ece\u5c5e\u4e8e":ctx.moveTo(cx,cy-8);ctx.lineTo(cx+7,cy);ctx.lineTo(cx,cy+8);ctx.lineTo(cx-7,cy);ctx.closePath();ctx.stroke();break;
    case"\u8986\u76d6":ctx.arc(cx-4,cy,8,-Math.PI/2,Math.PI/2);ctx.stroke();break;
    case"\u5e94\u7528\u4e8e":ctx.moveTo(cx+7,cy);ctx.lineTo(cx-5,cy-7);ctx.lineTo(cx-5,cy+7);ctx.closePath();ctx.stroke();break;
    case"\u89e3\u91ca":ctx.arc(cx,cy,8,0,Math.PI*2);ctx.stroke();ctx.font='bold 10px sans-serif';ctx.textAlign="center";ctx.textBaseline="middle";ctx.fillText("i",cx,cy+1);break;
    case"\u6d4b\u91cf":ctx.lineWidth=2;ctx.moveTo(cx-6,cy-5);ctx.lineTo(cx+6,cy-5);ctx.moveTo(cx-8,cy);ctx.lineTo(cx+8,cy);ctx.moveTo(cx-6,cy+5);ctx.lineTo(cx+6,cy+5);ctx.stroke();break;
    case"\u5f71\u54cd":ctx.moveTo(cx+7,cy);ctx.lineTo(cx+3,cy-5);ctx.moveTo(cx+7,cy);ctx.lineTo(cx+3,cy+5);ctx.moveTo(cx-7,cy);ctx.lineTo(cx-3,cy-5);ctx.moveTo(cx-7,cy);ctx.lineTo(cx-3,cy+5);ctx.stroke();break;
    case"\u6f14\u53d8":ctx.arc(cx-1,cy,5,-Math.PI*0.7,Math.PI*0.7);ctx.moveTo(cx+5,cy-4);ctx.lineTo(cx+8,cy-2);ctx.lineTo(cx+3,cy-1);ctx.closePath();ctx.fill();break;
  }return c;
}
...
window.createGraph3D=function(container,data,nodeIndex){
  ...
  data.relations.forEach(function(rel){
    ...
    /* small icons */
    ds.scale.set(0.22,0.22,1);ds.renderOrder=2;ds.userData.relType=rel.type;
    ds.position.copy(_vA).add(_vB).multiplyScalar(0.5);
    ds.userData.srcGrade=tube.userData.srcGrade;ds.userData.tgtGrade=tube.userData.tgtGrade;
  ...
  function focusGrade(gname){
    ...
    decors.forEach(function(d){d.visible=(d.userData.srcGrade===gname&&d.userData.tgtGrade===gname);});
    var sp=camera.position.clone(),ep=new THREE.Vector3(0,0,z+14);
  ...
  function unfocusGrade(){
    ...
    tubes.forEach(function(t){t.visible=true;});decors.forEach(function(d){d.visible=true;});
    var sp=camera.position.clone(),ep=new THREE.Vector3(20,16,20);
  ...
  renderer.domElement.addEventListener("pointerup",function(e){
    ...
    if(hits.length>0){var o=hits[0].object;while(o&&!o.userData.nodeId)o=o.parent;if(o&&o.userData.nodeId&&clickCb){clickCb(o.userData.nodeId);}}
  });
  ...
  function highlightNode(nid){
    ...
    for(var id in sps){if(rel.has(id)){sps[id].material.opacity=1;if(lbs[id])lbs[id].material.opacity=1;}else{sps[id].material.opacity=0.15;if(lbs[id])lbs[id].material.opacity=0.08;}}
  ...
  applyTheme("day");
  function anim(){requestAnimationFrame(anim);if(autoRot&&!userInt)mg.rotation.y+=0.0015;controls.update();renderer.render(scene,camera);}
  anim();
  return{
